...
public class MatrixMultiply extends MatrixExecutor {
	...
	public MatrixAnswer execute() {
		...
		Matrix an = new Matrix(size_first.x, size_second.y, super.x.name() + " times " + super.y.name());
		MatrixWork work = new MatrixWork(size_first.x, size_second.y, super.x.name() + " times " + super.y.name());
		for (int i = 0; i < size_first.x; i++) {
			for (int k = 0; k < size_second.y; k++) {
				...
				for (int j = 0; j < size_second.x; j++) {
					...
					str += j == 0 ? "(" + x.get(i, j) + " * " + this.y.get(j, k) + ")"
							: " + (" + x.get(i, j) + " * " + this.y.get(j, k) + ")";
				}
				an.set(i, k, sum);
				work.set(i, k, str);
				sum = 0;
