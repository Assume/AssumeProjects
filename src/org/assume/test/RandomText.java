...
public class RandomText extends JFrame {
	...
	private static final char LOWER_BOUNDARY = 32; // lower ascii number
													// boundary
	private static final char UPPER_BOUNDARY = 126; // upper ascii number
													// boundary
	...
	public RandomText(String text, int numLabels) {
		...
		if (numLabels < 2)
			throw new IllegalArgumentException("Must have at least 2 labels");
	...
	// return a random char between the upper and lower boundaries (both
	// inclusive)
	private char randomChar() {
		return (char) (random.nextInt(UPPER_BOUNDARY - LOWER_BOUNDARY + 1) + LOWER_BOUNDARY);
	}

	// return a random String of the given length using the randomChar()
	// function
	...
	private boolean isFinished() {
		...
		for (int i = 0; i < finalText.length(); i++) {
			...
			labelText[numLabels - 1] = labelText[numLabels - 1].substring(0, i) + labelText[numLabels - 2].charAt(i)
					+ labelText[numLabels - 1].substring(i + 1);
		...
		labelText[0] = randomString(finalText.length()); // we need a new top
															// line
